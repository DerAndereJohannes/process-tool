<script lang="ts">
    ...
    const toggleSelected = () => {
            for (let i=0; i<$EntityStore.length; i++) {
                if (entity === $EntityStore[i]) {
                    $EntityStore[i].selected = !$EntityStore[i].selected;
                } else if (!$MultiStore){
...
<div class="card card-flex-container {entity.selected ? 'selected' : ''}" on:click={toggleSelected}> 
    <div class="card-item"><img class="icon" src="/assets/{entity.metadata.type}.svg" alt="icon"></div>
    <div class="card-item">
        <hgroup style="margin-bottom: 0;">
            <b>{entity.metadata.name}</b>
            <h5>{entity.metadata.type}</h5>
        </hgroup>
    </div>
</div>
